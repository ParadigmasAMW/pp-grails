	...
	<body>
		...
		<div id="show-comment" class="content scaffold-show" role="main">
			...
			<ol class="property-list comment">
				...
				<g:if test="${commentInstance?.who}">
				<li class="fieldcontain">
					<span id="who-label" class="property-label"><g:message code="comment.who.label" default="Who" /></span>
					
						<span class="property-value" aria-labelledby="who-label"><g:link controller="commentator" action="show" id="${commentInstance?.who?.id}">${commentInstance?.who?.encodeAsHTML()}</g:link></span>
