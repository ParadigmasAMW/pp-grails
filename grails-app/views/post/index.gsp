		...
		<div id="list-post" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="dateTime" title="${message(code: 'post.dateTime.label', default: 'Date Time')}" />
				...
				<g:each in="${postInstanceList}" status="i" var="postInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: postInstance, field: "teaser")}</td>
					
						<td>${fieldValue(bean: postInstance, field: "body")}</td>
					
						<td><g:formatDate date="${postInstance.dateTime}" /></td>
